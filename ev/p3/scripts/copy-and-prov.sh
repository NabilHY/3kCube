...
# remote_exec <command...>
# Runs the given command (arbitrary argv, or a single quoted string) on the
# VM over SSH using the dedicated key. Allocates a pty (-t) so interactive
# output (progress bars, prompts inside the remote script) behaves normally.
remote_exec() {
    if [ "$#" -eq 0 ]; then
